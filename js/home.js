...
//弹出菜单
$(function () {
    $('.contProMore').mouseover(function () {
        var dq = $(this).parent().parent('.p1').index();
        var p1Left = $(this).position().left - 400 + 'px';

        $('.cont_pro_menu').eq(dq).css('left', p1Left);
        $('.cont_pro_menu').eq(dq).fadeIn(600);
    });
    $('.cont_pro_menu').hover(
